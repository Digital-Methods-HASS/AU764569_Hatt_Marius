...
Western_Europe %>% 
  ...
  xlab("Year") +
  ylab("Number of homicides per 100k people") +
  ggtitle("Homicide rates in Europe 1300-2000, AD") +
  labs(color = "Country") +  
  theme_bw(base_size = 11) +
...
```{r eval = TRUE}
kongerækken <- read_csv("data/kongeraekke - kongeraekke (1).csv")

kongerækken %>% 
  mutate(duration = fratreadelse - Koringering) %>% 
  mutate(midyear = fratreadelse - (fratreadelse - Koringering)/2) %>% 
...
The length of the duration of reign of Kings and Queens in Denmark, has increased since the middle 12th century. This can be due to the fact, that living standards and life expectancy has been increased over time aswell. The reason for low point of "duration of reign in years", matches well with the faceted plot above, which indicates that where the duration of reign is lowest, the homocide rate is highest.
...
From what can be observed from the data visualisations, is that there appeares to be a general decline in homicides in Europe from year 1300 AD to 2000 AD. Compared to the plot of duration of reign among danish kings and queens, we can see that the duration of reign increases while homocides per 100k decreases. This more or less concludes, that we are becoming more civilized. The duration of reign do reveal, that in 900 AD, the duration of reign was more than in 1150-1200 AD. The homocide data does not go so far back, so from the duration of reign data, we can assume or guess that we became less civilized from 900 AD to 1200 AD.
